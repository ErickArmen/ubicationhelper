...
class MarkerAnimator(val googleMap: GoogleMap) {
    ...
    /*fun markerAnimation(marker: Marker?){
        val startValues = ArrayList<Double?>()
        startValues.add(marker?.position?.latitude)
        startValues.add(marker?.position?.longitude)
        val endValues = ArrayList<Double?>()
        endValues.add()
    }*/
}